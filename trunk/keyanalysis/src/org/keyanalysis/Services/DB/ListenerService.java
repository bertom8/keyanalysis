...
public class ListenerService {
	...
	public static <T> ClickListener getRemoveButtonListener(Table table) {
		...
		return new ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				if(table.getValue()==null){
					...
					ConfirmDialog.show(UI.getCurrent(), "Delete", "Are you sure you want to delete the seleceted item?", "OK", "CANCEL", 
						...
						public void onClose(ConfirmDialog dialog) {
							if (dialog.isConfirmed()) {								
								...
								ProcessService.removeDir((String)jpa.getItem(itemId).getItemProperty("filePath").getValue());
	 ...
	 */
	@SuppressWarnings("unchecked")
	public static <T> ClickListener getCopyButtonListener(Table table) {
		JPAContainer<T> jpa = (JPAContainer<T>)table.getContainerDataSource();
		return new ClickListener() {
			private static final long serialVersionUID = 1L;
			...
			public void buttonClick(ClickEvent event) {
				if(table.getValue()==null){
					...
					Object itemId = table.getValue();
					Item item = (Item)jpa.getItem(itemId).getEntity();
