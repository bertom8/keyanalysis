...
/**
 * This is a fast implementation of the Skein-512-512 hash function. All other
 * modes are not tested and supported. It is compatible with the revised
...
public class Skein512 {
	...
	private final int hashBitCount;
	// current byte count in the buffer
	private int byteCount;
	// tweak words: tweak0=byte count, tweak1=flags
	private long tweak0, tweak1;
	// chaining variables
	private final long[] x = new long[WORDS];
	// partial block buffer (8-byte aligned)
	private final byte[] buffer = new byte[BYTES];
	// key schedule: tweak
	private final long[] tweakSchedule = new long[5];
	// key schedule: chaining variables
	private final long[] keySchedule = new long[17];

	// build/process the configuration block (only done once)
	private Skein512(final int hashBitCount) {
		this.hashBitCount = hashBitCount;
		this.startNewType(TYPE_CONFIG | T1_FLAG_FINAL);
		// set the schema, version
		final long[] w = new long[] { SCHEMA_VERSION, hashBitCount };
		// compute the initial chaining values from the configuration block
		this.setBytes(this.buffer, w, 2 * 8);
		this.processBlock(this.buffer, 0, 1, 4 * WORDS);
		// the chaining vars (x) are now initialized for the given hashBitLen.
		// set up to process the data message portion of the hash (default)
		// buffer starts out empty
		this.startNewType(TYPE_MESSAGE);
	}
	...
	/**
	 * Calculate the hash code of the given message. Each bit in the message is
	 * processed.
	 *
	 * @param msg
	 *            the message
	 ...
	 */
	public static void hash(final byte[] msg, final byte[] digest) {
	 ...
	 *            the resulting hash code
	 */
	public static void hash(final byte[] msg, final int bitCount, final byte[] digest) {
		int byteCount = bitCount >>> 3;
		if ((bitCount & 7) != 0) {
			final int mask = 1 << (7 - (bitCount & 7));
			msg[byteCount] = (byte) ((msg[byteCount] & (-mask)) | mask);
			byteCount++;
		}
		final Skein512 instance = new Skein512();
	...
	private void update(final byte[] msg, int len) {
		int pos = 0;
		...
		if (len + this.byteCount > BYTES) {
			...
			if (this.byteCount != 0) {
				...
				final int n = BYTES - this.byteCount;
				if (n != 0) {
				...
				this.processBlock(this.buffer, 0, 1, BYTES);
				this.byteCount = 0;
			}
			...
			if (len > BYTES) {
				...
				final int n = (len - 1) / BYTES;
				this.processBlock(msg, pos, n, BYTES);
				len -= n * BYTES;
				pos += n * BYTES;
			}
		}
	...
	private void finalize(final byte[] hash) {
		// tag as the final block
		this.tweak1 |= T1_FLAG_FINAL;
		// zero pad if necessary
		if (this.byteCount < BYTES) {
			Arrays.fill(this.buffer, this.byteCount, BYTES, (byte) 0);
		}
		// process the final block
		this.processBlock(this.buffer, 0, 1, this.byteCount);
		// now output the result
		// zero out the buffer, so it can hold the counter
		Arrays.fill(this.buffer, (byte) 0);
		// up to 512 bits are supported
		// build the counter block
		this.startNewType(TYPE_OUT | T1_FLAG_FINAL);
		// run "counter mode"
		this.processBlock(this.buffer, 0, 1, 8);
		// "output" the counter mode bytes
		this.setBytes(hash, this.x, (this.hashBitCount + 7) >> 3);
	}

	// set up for starting with a new type
	private void startNewType(final long type) {
		this.tweak0 = 0;
		this.tweak1 = T1_FLAG_FIRST | type;
	}

	private void processBlock(final byte[] block, int off, int blocks, final int bytes) {
		while (blocks-- > 0) {
			// this implementation supports 2**64 input bytes (no carry out
			// here)
			// update processed length
			final long[] ts = this.tweakSchedule;
			this.tweak0 += bytes;
			final int[] mod3 = MOD3;
			final int[] mod9 = MOD9;
			...
			final long[] c = this.x;
			final long[] ks = this.keySchedule;
			...
			long x0 = (c[0] = this.getLong(block, off)) + ks[0];
			long x1 = (c[1] = this.getLong(block, off + 8)) + ks[1];
			long x2 = (c[2] = this.getLong(block, off + 16)) + ks[2];
			long x3 = (c[3] = this.getLong(block, off + 24)) + ks[3];
			long x4 = (c[4] = this.getLong(block, off + 32)) + ks[4];
			long x5 = (c[5] = this.getLong(block, off + 40)) + ks[5] + this.tweak0;
			long x6 = (c[6] = this.getLong(block, off + 48)) + ks[6] + this.tweak1;
			long x7 = (c[7] = this.getLong(block, off + 56)) + ks[7];
			// unroll 8 rounds
			for (int r = 1; r <= ROUNDS / 4; r += 2) {
				final int rm9 = mod9[r], rm3 = mod3[r];
				x1 = this.rotlXor(x1, R00, x0 += x1);
				x3 = this.rotlXor(x3, R01, x2 += x3);
				x5 = this.rotlXor(x5, R02, x4 += x5);
				x7 = this.rotlXor(x7, R03, x6 += x7);
				x1 = this.rotlXor(x1, R10, x2 += x1);
				x7 = this.rotlXor(x7, R11, x4 += x7);
				x5 = this.rotlXor(x5, R12, x6 += x5);
				x3 = this.rotlXor(x3, R13, x0 += x3);
				x1 = this.rotlXor(x1, R20, x4 += x1);
				x3 = this.rotlXor(x3, R21, x6 += x3);
				x5 = this.rotlXor(x5, R22, x0 += x5);
				x7 = this.rotlXor(x7, R23, x2 += x7);
				x1 = this.rotlXor(x1, R30, x6 += x1) + ks[rm9 + 1];
				x7 = this.rotlXor(x7, R31, x0 += x7) + ks[rm9 + 7] + r;
				x5 = this.rotlXor(x5, R32, x2 += x5) + ks[rm9 + 5] + ts[rm3];
				x3 = this.rotlXor(x3, R33, x4 += x3) + ks[rm9 + 3];
				x1 = this.rotlXor(x1, R40, x0 += x1 + ks[rm9]);
				x3 = this.rotlXor(x3, R41, x2 += x3 + ks[rm9 + 2]);
				x5 = this.rotlXor(x5, R42, x4 += x5 + ks[rm9 + 4]);
				x7 = this.rotlXor(x7, R43, x6 += x7 + ks[rm9 + 6] + ts[rm3 + 1]);
				x1 = this.rotlXor(x1, R50, x2 += x1);
				x7 = this.rotlXor(x7, R51, x4 += x7);
				x5 = this.rotlXor(x5, R52, x6 += x5);
				x3 = this.rotlXor(x3, R53, x0 += x3);
				x1 = this.rotlXor(x1, R60, x4 += x1);
				x3 = this.rotlXor(x3, R61, x6 += x3);
				x5 = this.rotlXor(x5, R62, x0 += x5);
				x7 = this.rotlXor(x7, R63, x2 += x7);
				x1 = this.rotlXor(x1, R70, x6 += x1) + ks[rm9 + 2];
				x7 = this.rotlXor(x7, R71, x0 += x7) + ks[rm9 + 8] + r + 1;
				x5 = this.rotlXor(x5, R72, x2 += x5) + ks[rm9 + 6] + ts[rm3 + 1];
				x3 = this.rotlXor(x3, R73, x4 += x3) + ks[rm9 + 4];
				x0 += ks[rm9 + 1];
				x2 += ks[rm9 + 3];
				x4 += ks[rm9 + 5];
	...
	private long rotlXor(final long x, final int n, final long xor) {
		return ((x << n) | (x >>> (64 - n))) ^ xor;
	}

	private void setBytes(final byte[] dst, final long[] src, final int byteCount) {
	...
	private long getLong(final byte[] b, final int i) {
		if (i >= b.length + 8) {
